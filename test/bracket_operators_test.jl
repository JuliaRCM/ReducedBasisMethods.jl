@testset "Bracket Tests - ϕ and h formulation" begin
    ...
    P_h = LinearMap(_closure_apply_P_h!, _closure_apply_P_hᵀ!,
        n₁*n₂; ismutating = true, issymmetric = false)
    ...
    P_ϕ = LinearMap(_closure_apply_P_ϕ!, _closure_apply_P_ϕᵀ!,
        n₁*n₂; ismutating = true, issymmetric = false)
...
@testset "Bracket Tests - analytical solution" begin
    ...
    fh = vec([32 * _v * sin(_x * 2π)^2 * (cos(_v * 2π) - 1) * exp(- 16 * _v^2) +
              2π * cos(_x * 2π)^2 * sin(_v * 2π) * exp(- 16 * _v^2) for _x in x, _v in v])
