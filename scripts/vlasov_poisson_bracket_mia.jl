...
struct ReducedTensor{DT, PT <: AbstractMultiIndexArray{DT, 3}, PM1, PM2} <:
       AbstractArray{DT, 3}
    tensor::PT
    projection_i::PM1
    projection_j::PM2
    ...
    function ReducedTensor(tensor::AbstractMultiIndexArray{DT}, Pi::PM1, Pj::PM2) where {
            DT, PM1, PM2}
...
function Base.size(rt::ReducedTensor)
    (size(rt.projection_i, 1), size(rt.projection_j, 2), size(rt.tensor, 3))
end
...
struct PoissonOperator{DT, PT, HT} <: AbstractMatrix{DT}
    ...
    function PoissonOperator(tensor::Union{AbstractMultiIndexArray{DT}, ReducedTensor{DT}},
            h::HT) where {DT, HT}
...
function test_arakawa()
    ...
    P_tens = MultiIndexLazyArray(
        Float64, (inds...) -> arakawa(inds...), (nx, nv), (nx, nv), (nx, nv))
