...
IP = VPIntegratorParameters(
    integrator.dt, integrator.nₜ, integrator.nₛ, integrator.nₕ, integrator.nₚ)
IC = VPIntegratorCache(IP)

# test set
TS = TrainingSet(
    particles, poisson, integrator.nₜ+1, params, pspace, IntegratorParameters(IP, pspace))
SS = TS.snapshots
...
@time for p in eachindex(pspace)
    ...
    reduced_integrate_vp(particles, Ψₚ, lparams, rfield, RSS,
        IntegratorParameters(IP, pspace), RIC, p; save = true)
end
